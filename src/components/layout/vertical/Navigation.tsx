...
const Navigation = (navigationProps: Props) => {
  // Props
  const { dictionary, mode, systemMode, lang, props } = navigationProps
  ...
  return (
    ...
    >
      ...
      <NavHeader>
        <Link href={getLocalizedUrl('/', locale as Locale)} className="flex justify-center flex-1">
          <Logo />
        </Link>
        {!(isCollapsed && !isHovered) && (
          <NavCollapseIcons
            lockedIcon={<i className='tabler-arrow-bar-left text-xl' />}
            unlockedIcon={<i className='tabler-arrow-bar-right text-xl' />}
      ...
      <VerticalMenu dictionary={dictionary} lang={lang} scrollMenu={scrollMenu} props={props} />
    </VerticalNav>
  )
}
