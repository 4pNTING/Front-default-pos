...
// === TEMPORARY ROLE TYPE ===
// Define the properties used in the table plus any for flexibility.
interface TempRole {
  _id: string; // Used as NO and row key
  systemCode?: string | null;
  laName?: string | null;
  enName?: string | null;
  menus: Array<any>; // Used to check length
...
// 🛑 Note: Role is now the temporary type above
...
const RoleListTable = ({ props }: { props: RolesListProps }) => {
  // Hooks
  // We explicitly tell TS that roles is an array of our temporary Role type
  const { loading, roles, queryRoles, setToUpdate } = useRoleStore() as {
    loading: boolean;
    roles: Role[];
    queryRoles: any;
    setToUpdate: any;
      ...
      columnHelper.accessor("action", {
        ...
        cell: ({ row }) => (
          <div className="flex items-center justify-center gap-2">
            ...
            <IconButton
              onClick={() => {
                setToUpdate(row?.original as Role); // 🛑 Cast original to Role (TempRole)
                openModal(roleMTs.update);
              }}
            >
